...
const CATEGORY_STYLE = {
  ...
  round: 'border-emerald-500/40 bg-emerald-500/10 text-ufa-success',
  cup: 'border-ufa-gold/40 bg-ufa-gold/10 text-ufa-gold',
  awards: 'border-sky-400/40 bg-sky-400/10 text-ufa-info',
  power_rankings: 'border-violet-400/40 bg-violet-500/10 text-ufa-special',
  transfer: 'border-amber-400/40 bg-amber-500/10 text-ufa-gold',
  rumor: 'border-fuchsia-400/40 bg-fuchsia-500/10 text-ufa-special',
  feature: 'border-ufa-border bg-ufa-bg text-ufa-muted',
  breaking: 'border-red-400/40 bg-red-500/10 text-ufa-danger',
  ...
  return (
    <div className="space-y-6 league-fade-in">
      <div className="overflow-hidden rounded-sm border border-ufa-border bg-ufa-panel  ">
        <div className="relative border-b border-ufa-border bg-ufa-panel px-5 py-6 sm:px-8">
      ...
      {filtered.length === 0 && !selected ? (
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
                  ...
                  <li key={article.id}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <span
                          className={`rounded border px-1.5 py-0.5 text-[11px] font-semibold uppercase ${style}`}
                        >
          ...
          <article className="rounded-sm border border-ufa-border bg-ufa-panel p-5   sm:p-7">
            {selected ? (
              <>
                ...
                {selected.effectsSummary || selected.effectsSummaryEn ? (
                  <p className="mt-4 rounded-sm border border-ufa-gold/30 bg-ufa-gold/5 px-3 py-2 text-xs text-ufa-gold">
                ...
                {selected.tags?.length ? (
                  <div className="mt-5 flex flex-wrap gap-1.5">
                    {selected.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded border border-ufa-border px-2 py-0.5 text-[11px] uppercase tracking-wide text-ufa-muted"
                      >
                        #{tag}
                      </span>
                    ))}
