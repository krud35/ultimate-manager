...
export default function ClubWelcomeMessage({ welcome, lang }) {
  ...
  return <div className="mt-4 space-y-5">
    <div className="rounded-sm border border-ufa-accent/30 bg-ufa-accent/5 p-4">
      <p className="text-xs text-ufa-muted">{en ? 'Welcome to' : 'Witamy w'} {welcome.teamName}</p>
      <p className="mt-3 text-[11px] font-semibold uppercase tracking-widest text-ufa-accent">{en ? 'Club strategy' : 'Strategia klubu'}</p>
    ...
    <section aria-label={en ? 'Board objectives' : 'Cele zarządu'}>
      ...
      <ul className="grid gap-2 sm:grid-cols-2">
        {objectives.map(o => <li key={o.id} className={`min-w-0 rounded-sm border p-3 ${o.priority === 1 ? 'border-ufa-gold/40 bg-ufa-gold/5 sm:col-span-2' : 'border-ufa-border bg-ufa-bg/40'}`}>
          <div className="flex flex-wrap items-center justify-between gap-2"><span className="text-xs text-ufa-muted">{o.label}</span><span className={`rounded px-2 py-0.5 text-[11px] font-semibold ${o.priority === 1 ? 'bg-ufa-gold/15 text-ufa-gold' : 'bg-ufa-panel text-ufa-muted'}`}>{en ? 'Priority' : 'Priorytet'} {o.priority}</span></div>
    ...
    <section aria-label={en ? 'Available funds' : 'Dostępne środki'}>
      ...
      <dl className="grid gap-2 sm:grid-cols-2">{tiles.map(([label, value, note]) => <div key={label} className="min-w-0 rounded-sm border border-ufa-border bg-ufa-bg/40 p-3">
        <dt className="text-xs text-ufa-muted">{label}</dt><dd className={`mt-1 break-words text-xl font-semibold tabular-nums ${value < 0 ? 'text-ufa-danger' : 'text-ufa-text'}`}>{formatUsd(value)}</dd>
