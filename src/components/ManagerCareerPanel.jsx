...
export default function ManagerCareerPanel({career,onUpdate,onWait}) {
 ...
 return <section className="space-y-4 um-section text-ufa-text">
  ...
  {confirm&&<div role="alertdialog" aria-label={en?'Confirm job change':'Potwierdź zmianę pracy'} className="rounded-sm border border-ufa-gold p-4">
   ...
   <div className="mt-3 flex gap-3"><button type="button" className="rounded bg-ufa-accent px-3 py-2 text-ufa-on-accent" onClick={execute}>{en?'Confirm':'Potwierdź'}</button><button type="button" className="rounded border border-ufa-border px-3 py-2" onClick={()=>setConfirm(null)}>{en?'Cancel':'Anuluj'}</button></div>
  </div>}
