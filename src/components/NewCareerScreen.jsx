...
export default function NewCareerScreen({ slotIndex, lang = 'pl', onCancel, onCreate, submitting = false, externalError = '' }) {
  ...
  const [eucsOptions, setEucsOptions] = useState(null)
  ...
  const international = Object.entries(cups).filter(([id]) => competition === 'domestic' || competition === 'ufa' && id === 'nationals')
  const setupOptions = competition === 'eucs' ? eucsOptions : ufaOptions
  ...
  function submit() {
    ...
    if (competition === 'domestic' ? !selectedTeam : !setupOptions) return setStep(3)
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8 sm:px-6 league-fade-in">
    ...
    {step === 1 && <div className="grid gap-4 md:grid-cols-3">{[['eucs', 'UltiLeague (EUCS teams)', tr('48 klubów EUCS w trzech poziomach po 16 drużyn. Awanse, spadki i Puchar Piramidy.', '48 EUCS clubs across three tiers of 16 teams. Promotion, relegation and the Pyramid Cup.')], ['domestic', tr('Ligi krajowe', 'Domestic leagues'), tr('Świat lig krajowych, krajowych pucharów i rozgrywek międzynarodowych.', 'Build a world of national leagues, domestic cups and international competitions.')], ['ufa', 'UFA', tr('Oddzielna liga północnoamerykańska z historycznymi sezonami i ustawieniami składów.', 'A separate North American league with historical seasons and roster settings.')]].map(([id, title, description]) => <button type="button" key={id} aria-pressed={competition === id} onClick={() => setCompetition(id)} className={`${panel} career-choice text-left ${competition === id ? 'border-ufa-accent bg-ufa-accent/10' : ''}`}><span className="mb-3 flex justify-end"><SelectionIndicator selected={competition === id} lang={lang} /></span><h2 className="text-xl font-semibold text-ufa-text">{title}</h2><p className="mt-3 text-sm text-ufa-muted">{description}</p></button>)}</div>}

    {step === 2 && <div className="space-y-5">
      {competition === 'eucs' && <section className={panel}>
        <h2 className="text-xl font-semibold text-ufa-text">UltiLeague (EUCS teams)</h2>
        <p className="mt-3 text-sm text-ufa-muted">{tr('Osobny świat 48 klubów EUCS: UltiLeague 1, 2 i 3 — po 16 drużyn na poziom. Wybór klubu określa Twój poziom startowy.', 'A separate world of 48 EUCS clubs: UltiLeague 1, 2 and 3, with 16 teams per tier. Your club determines your starting tier.')}</p>
        <p className="mt-3 text-sm text-ufa-muted">{tr('Co sezon po 3 drużyny awansują i spadają między poziomami. Wszystkie kluby uczestniczą w Pucharze Piramidy. Ten tryb obejmuje wyłącznie rozgrywki UltiLeague, bez lig krajowych.', 'Each season, 3 teams move up and down between tiers. All clubs compete in the Pyramid Cup. This mode includes only UltiLeague competitions, with no domestic leagues.')}</p>
      </section>}
      {competition === 'domestic' && <>
        <div className={panel} aria-live="polite"><h2 className="font-semibold text-ufa-text">{tr('Liga główna', 'Main league')}: {country(effectiveConfig.mainCountryId)} · + {effectiveConfig.additionalCountryIds.length} {tr('dodatkowe aktywne', 'additional active')}</h2><p className="mt-2 text-sm text-ufa-muted">{tr('Liga główna i dodatkowe ligi umożliwiają transfery oraz zmianę klubu. Liczba dodatkowych aktywnych lig jest nieograniczona. Możesz wyłączyć symulację pozostałych lig.', 'The main and additional active leagues allow transfers and changing clubs. Additional active leagues are unlimited. You can disable simulation of the remaining leagues.')}</p><WorldSimulationOptions config={effectiveConfig} lang={lang} onChange={setWorldConfig} /><p className="mt-1 text-sm text-ufa-muted">{activeLeagueCount} {tr('grywalnych lig', 'playable leagues')} · {DOMESTIC_LEAGUES.filter(l => effectiveConfig.leagues[l.id] === 'background').length} {tr('lig w tle', 'background leagues')} · {cost.clubs} {tr('klubów', 'clubs')} · {cost.players} {tr('zawodników', 'players')}</p><p className="mt-2 text-xs text-ufa-muted">{tr('Wskaźnik porównawczy, bez pomiaru czasu na tym urządzeniu. Niższy poziom wymaga wszystkich wyższych lig. Grupy regionalne włączane są razem. Awanse i spadki obejmują też ligi tła. Liga Twojego klubu zawsze pozostaje aktywna.', 'A relative estimate, without timing this device. Lower tiers require all higher tiers. Regional groups are enabled together. Promotion and relegation include background tiers. Your club’s league always stays active.')}</p></div>
    ...
    {competition !== 'domestic' && <div hidden={step !== 3}><UfaCareerSetup key={competition} initialCompetition={competition} slotIndex={slotIndex} lang={lang} embedded initialManagerName={managerName} onCancel={() => setStep(2)} onCreate={options => { if (competition === 'eucs') setEucsOptions(options); else setUfaOptions(options); setError(''); setStep(4) }} /></div>}

    {step === 4 && <div className="space-y-5"><div className={panel}><h2 className="text-xl font-semibold text-ufa-text">{managerName}</h2><p className="mt-2 text-sm text-ufa-muted">{country(identity.nationality)} · {identity.age} {tr('lat', 'years old')}</p><p className="mt-4 text-lg text-ufa-text">{competition === 'domestic' ? selectedTeam?.name : (setupOptions?.playerTeamName ?? setupOptions?.playerTeamId)}</p><p className="mt-1 text-sm text-ufa-muted">{competition === 'domestic' ? `${selectedLeague?.name} · 2026/27` : competition === 'eucs' ? `UltiLeague (EUCS teams) · ${setupOptions?.seasonYear}/${String((setupOptions?.seasonYear ?? 2026) + 1).slice(-2)}` : `UFA · ${setupOptions?.seasonYear}`}</p>{competition === 'domestic' && <p className="mt-3 text-sm text-ufa-muted">{activeLeagueCount} {tr('grywalnych lig · Krajowe puchary aktywne', 'playable leagues · Domestic cups included')}</p>}<p className="mt-3 text-sm text-ufa-muted">{tr('Rozgrywki międzynarodowe', 'International competitions')}: {international.filter(([id]) => worldConfig.international[id]).map(([, labels]) => labels[en ? 1 : 0]).join(', ') || tr('Wyłączone', 'Disabled')}</p></div><Attributes profile={profile} lang={lang} /><p className="text-sm text-ufa-muted">{competition === 'domestic' ? tr('Świat zostanie wygenerowany po rozpoczęciu gry. Dodatkowe aktywne ligi możesz zmienić na następny sezon.', 'Your world will be generated when you start. You can change additional active leagues for the next season.') : tr('Świat zostanie wygenerowany po rozpoczęciu gry.', 'Your world will be generated when you start.')}</p></div>}
    ...
    <div className="mt-8 flex flex-wrap gap-3">
      ...
      {step < 4 && !(step === 3 && competition !== 'domestic') && <button type="button" className="um-button um-button--primary" onClick={advance}>{tr('Dalej', 'Continue')}</button>}
