...
export default function UfaCareerSetup({
  ...
  const teams = useMemo(() => leaguePreview.teams ?? [], [leaguePreview.teams])
  const benchTeams = useMemo(() => leaguePreview.benchTeams ?? [], [leaguePreview.benchTeams])
  ...
  function handleSubmit(e) {
    ...
    const name = (embedded ? initialManagerName : managerName).trim()
    if (!name) {
  ...
  return (
    <div className={embedded ? 'space-y-5' : 'mx-auto flex min-h-screen max-w-3xl flex-col px-4 py-10 sm:px-6 league-fade-in'}>
      {!embedded && <button
        type="button"
        onClick={onCancel}
      ...
      {!embedded && <h1 className="um-page-title text-ufa-text">{t.newTitle(slotIndex + 1)}</h1>}
      ...
      {!embedded && <fieldset className="mt-6">
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-8">
        {!embedded && <label className="block">
            ...
            <div className="mt-3 space-y-6">
              {eucsTiers.map(({ tier, teams }) => (
                <div key={tier}>
                  ...
                  className={`career-choice rounded-sm border p-4 transition-all ${
                            active
                              ? 'border-ufa-accent bg-ufa-accent/10  '
                        ...
                        >
                          ...
                          >
                            <span className="mb-3 flex justify-end"><SelectionIndicator selected={active} lang={lang} /></span>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {submitting ? t.startingCareer : embedded ? (lang === 'en' ? 'Review career' : 'Podsumowanie kariery') : t.startCareer}
          </button>
          <button
            type="button"
            onClick={onCancel}
