import { focusedRole, changeFocusedRole, focusedLimit } from '../data/focusedWorld.js'
...
export default function WorldCountryChoice({ country, config, lang, onChange, lockMain = false }) {
  ...
  return <div className="rounded-sm border border-ufa-border bg-ufa-panel p-4">
    ...
    <div className="mt-3 grid gap-3 sm:grid-cols-2">
      <label className="text-sm text-ufa-muted">{tr('Symulacja', 'Simulation')}
        <select aria-label={`${tr('Symulacja', 'Simulation')}: ${name}`} className={selectClass} value={focused?role:mode} onChange={e => {
          ...
          {focused ? <>
            <option value="main" disabled={(lockMain && role!=="main") || (country.id==='fr'&&effective.additionalCountryIds.filter(id=>id!=='fr').length>3)}>{tr("Liga główna", "Main league")}</option>
            <option value="active" disabled={role==='main'||(role==='background'&&(effective.additionalCountryIds.length >= focusedLimit(effective) || (country.id==='fr'&&effective.additionalCountryIds.length>=3)))}>{tr("Dodatkowa aktywna", "Additional active")}</option>
            <option value="background" disabled={role==='main'}>{tr("Tło", "Background")}</option>
          </> : <>
    ...
    <ul className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ufa-muted">{country.leagues.map(l=><li key={l.id} className={!['off','background'].includes(effective.leagues[l.id])?'font-semibold text-ufa-accent':''}>{effective.leagues[l.id]!=='off'?'✓ ':''}{l.frenchPyramid ? l.name : levelName(l.tier)}: {l.teams.length} {tr('klubów', 'clubs')}</li>)}</ul>
  </div>
}
