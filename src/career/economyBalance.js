...
export const FINANCE_BALANCE_VERSION = 3
...
export function referenceClubCosts(team) {
  ...
  return Math.round((28 * referenceWeeklyWage(standardOvr) * 1.2 + 60_000) * 52)
}
...
export function matchCommercials(team, { won = false, isHome = true, neutral = false } = {}) {
  ...
  const following = Math.max(250, team?.fans?.size ?? 3000) * 15
  ...
  const matchCosts = isHome ? Math.round(30_000 + attendance * 5) : 0
