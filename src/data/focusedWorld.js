...
export function focusedSelection(input, countries) {
  ...
  const config={...input,simulationModel:'focused',mainCountryId,additionalCountryIds,backgroundSimulation:input.backgroundSimulation!==false}
  ...
  for(const c of countries) {
    ...
    for(const l of c.leagues)config.leagues[l.id]=active.has(c.id)&&l.tier<=depth?'playable':config.backgroundSimulation?'background':'off'
  }
  return config
}
