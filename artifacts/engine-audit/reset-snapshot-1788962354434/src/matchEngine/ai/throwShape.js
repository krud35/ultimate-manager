...
import { createDiscTrajectory, sampleDiscTrajectory } from './discTrajectory.js'
import {
  THROW_ARC,
  horizontalReachM,
  maxAerialReachM,
...
export function scoreThrowShape(candidate, ctx) {
  ...
  const flightSec = Math.min(9500, Math.max(80, Math.round(baseFlightMs * hangMult))) / 1000
  const amplitudeM = baseAmplitudeM * (CURVE_AMP_MULT[candidate.curve] ?? 1)
  const sign = candidate.curveSign ?? 1
  let plan = createDiscTrajectory({ fromX, fromY, toX, toY,
    totalFlightMs: flightSec * 1000, startHeightM: releaseHeightM, peakHeightM: peakM,
    endHeightM: deliveryHeightM, amplitudeM, curveSign: sign, spinRadSec, wind })
  const predictedEnd = sampleDiscTrajectory(plan, plan.totalMs)
  const compensation = { dx: (toX - predictedEnd.x) * Math.min(1, Math.max(0, windControl / 100)),
    dy: (toY - predictedEnd.y) * Math.min(1, Math.max(0, windControl / 100)) }
  if (compensation.dx || compensation.dy) plan = createDiscTrajectory({ fromX, fromY,
    toX: toX + compensation.dx, toY: toY + compensation.dy, totalFlightMs: flightSec * 1000,
    startHeightM: releaseHeightM, peakHeightM: peakM, endHeightM: deliveryHeightM,
    amplitudeM, curveSign: sign, spinRadSec, wind })

  let laneCost = 0
  let worstGap = Infinity
  for (let i = 0; i < LANE_SAMPLES; i += 1) {
    const u = LANE_U_FROM + ((LANE_U_TO - LANE_U_FROM) * i) / (LANE_SAMPLES - 1)
    const { x, y, z } = sampleDiscTrajectory(plan, u * plan.totalMs)
...
export function chooseThrowShape(thrower, ctx) {
  ...
  const scoreCtx = { ...ctx, windControl: subStat(thrower, 'throwing', 'windControl'),
    spinRadSec: 25 + subStat(thrower, 'throwing', 'releaseControl') * 0.4,
    defenders: reachAnnotatedDefenders(ctx.defenders ?? []) }
