...
/** Klatka stanu dysku/markera. Skokowe liczenie nie może być interpolowane. */
export function sampleStateFromMotionTrace(trace, clipMs, clipActionMs) {
  const frames = trace?.frames
  if (!frames?.length) return null
  const totalMs = trace.totalMs ?? frames.at(-1).ms
  const ms = Math.max(0, Math.min(totalMs, clipMs / Math.max(1, clipActionMs) * totalMs))
  let lo = 0, hi = frames.length - 1
  while (lo < hi) {
    const mid = Math.ceil((lo + hi) / 2)
    if (frames[mid].ms <= ms) lo = mid
    else hi = mid - 1
  }
  return frames[lo]
}
