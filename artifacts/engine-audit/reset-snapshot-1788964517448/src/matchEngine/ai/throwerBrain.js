...
export const RESET_PASS_CALIBRATION = { enabled: false, speedReserve: 0.85, maxUncertaintyM: 0.15, lateAllowanceMs: 0 }
...
function applyLaneReadToOptions(thrower, considered, { throwerPos, offenseAgents = [], defenseAgents = [], rng, threshold, wind, stallCount, required = false }) {
  ...
  for (const option of considered.slice(0, limit)) {
    ...
    if (option.agent) {
      const safeReset = RESET_PASS_CALIBRATION.enabled && option.throwType === THROW_TYPE.DUMP_SWING && stallCount < 5
      const intercept = selectDiscIntercept({ agent: option.agent, player: option.player, role: 'offense',
        speed: maxSpeedMps(option.player) * (safeReset ? RESET_PASS_CALIBRATION.speedReserve : 1), elapsedMs: 0,
        totalMs: shape.plan.totalMs + (safeReset ? RESET_PASS_CALIBRATION.lateAllowanceMs : 12000),
      ...
      if (safeReset && uncertaintyM > RESET_PASS_CALIBRATION.maxUncertaintyM) option.score = -Infinity
