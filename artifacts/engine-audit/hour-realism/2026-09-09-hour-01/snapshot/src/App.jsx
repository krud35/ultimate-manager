...
import { useUiTheme } from './ui/UiThemeContext.jsx'
...
const MOBILE_NAV = [
  { id: 'hub', labelPl: 'Centrum', labelEn: 'Hub', Icon: IconHome },
  { id: 'roster', labelPl: 'Skład', labelEn: 'Roster', Icon: IconShirt },
  { id: 'match', labelPl: 'Mecz', labelEn: 'Match', Icon: IconNextDay },
  { id: 'menu', labelPl: 'Menu', labelEn: 'Menu', Icon: IconDots },
]
...
function SectionKicker({ children }) {
  return <p className="ufa-section-kicker">{children}</p>
}
...
export default function App() {
  ...
  if (screen === 'slots') {
    return (
      <>
        <ScreenBackground scene="career-select" enabled={false} />
  ...
  if (screen === 'new' && pendingSlot != null) {
    return (
      <>
        <ScreenBackground scene="career-new" enabled={false} />
        <div className="relative z-10 min-h-screen">
          <NewCareerScreen
  ...
  return (
    <>
    <ScreenBackground scene={sceneForTab(activeTab)} enabled={bgEnabled && activeTab === 'match'} />
    <div className="relative z-10 flex min-h-screen flex-col md:flex-row">
      <aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col border-r border-ufa-border bg-ufa-panel/85 md:flex">
        <div className="flex items-center gap-2.5 border-b border-ufa-border px-4 py-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[10px] text-ufa-muted">{career.managerName} · {userTeam?.name ?? ''}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 border-t border-ufa-border p-3">
          <div className="flex items-center gap-2">
            ...
            <ThemeSelect lang={uiLang} compact />
            <button
              type="button"
              onClick={toggleBg}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="ufa-main mx-auto w-full max-w-[1600px] flex-1 px-3 py-4 pb-28 sm:px-6 sm:py-6 md:pb-6 league-fade-in">
        {activeTab === 'hub' && (
          <LeagueHub
            league={league}
        ...
        >
          ...
          <div className="grid grid-cols-4">
            {MOBILE_NAV.map((item) => {
              ...
              const active = item.id === 'menu' ? mobileMenuOpen : activeTab === item.id
              const badge = item.id === 'hub' ? inboxUnread : item.id === 'menu' ? ultiworldUnread : 0
              return (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => item.id === 'menu' ? setMobileMenuOpen((open) => !open) : navigateTo(item.id)}
      ...
      {mobileMenuOpen && activeTab !== 'match' && (
        <div className="fixed inset-x-2 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-40 max-h-[70vh] overflow-y-auto border border-ufa-border bg-ufa-panel p-3 shadow-2xl md:hidden">
          <div className="mb-2 flex items-center justify-between border-b border-ufa-border pb-2">
            <p className="ufa-section-kicker mb-0 flex-1 border-0 pb-0">{uiLang === UI_LANG.EN ? 'All areas' : 'Wszystkie obszary'}</p>
            <button type="button" onClick={() => setMobileMenuOpen(false)} className="min-h-10 px-2 text-sm text-ufa-muted" aria-label={commonStrings(uiLang).close}>✕</button>
          </div>
          <div className="grid gap-1 sm:grid-cols-2">
            {NAV_CATEGORIES.flatMap((cat) => cat.items.map((item) => ({ ...item, category: cat }))).map((item) => (
              <button
                key={item.id}
                type="button"
                onClick={() => navigateTo(item.id)}
                className={`flex min-h-11 items-center justify-between border-l-2 px-3 text-left text-sm ${activeTab === item.id ? 'border-ufa-accent bg-ufa-panel-hover text-ufa-accent' : 'border-transparent text-ufa-text hover:bg-ufa-panel-hover'}`}
              >
                <span>{pickLabel(item, uiLang)}</span><span aria-hidden="true">→</span>
              </button>
            ))}
          </div>
        </div>
