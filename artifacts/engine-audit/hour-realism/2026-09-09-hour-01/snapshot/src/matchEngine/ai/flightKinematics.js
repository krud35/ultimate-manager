...
export const FLIGHT_APPROACH_CALIBRATION = { enabled: true, arrivalLeadSec: 0.15, stopBufferM: 0.08, readDelayScale: 1, interceptJumpClock: true,
  // Refresh a tracked disc between reactions; the initial reaction delay still applies.
  readIntervalMs: 80, readBlendScale: 0.5, purposefulLayout: true }
  ...
  if (agent.id === flight.receiverId || agent.id === flight.recoveryReceiverId) {
    const evidence = { elapsedMs, playerId: agent.id, observedAtMs: observation?.observedAtMs ?? null,
      actualReceiver: { x: agent.x, y: agent.y, z: agent.z ?? 0, vx: agent.vx ?? 0, vy: agent.vy ?? 0,
        diving: !!agent.diving, jumping: !!agent.jumping },
...
export function tickFlightContestAgent(agent, intercept, player, role, discSample, rng, speedMult = 1, flight = null) {
  ...
  // A reachable incoming disc does not need a dive towards its current position.
  // Diving locks velocity until landing, so that premature attempt can lose an easy catch.
  const needsLayout = !FLIGHT_APPROACH_CALIBRATION.purposefulLayout || role === 'defense' || !target.reachable
  if (needsLayout && !agent.layout && !occupied && movingToward && discSample.z < 1.35 && discSample.z > 0.15
