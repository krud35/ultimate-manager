...
export function selectDiscIntercept({ agent, player, role, speed, elapsedMs, totalMs, sample, blockers = [] }) {
  ...
  for (let ms = Math.min(totalMs, elapsedMs + 20); ; ms = Math.min(totalMs, ms + 40)) {
    ...
    let lateness = travelSec - availableSec + heightPenalty
    let motionGap = null
    if (ARRIVAL_CALIBRATION.kinematics && support.legal && heightPenalty === 0
      && travelSec <= availableSec && availableSec <= ARRIVAL_CALIBRATION.horizonSec) {
