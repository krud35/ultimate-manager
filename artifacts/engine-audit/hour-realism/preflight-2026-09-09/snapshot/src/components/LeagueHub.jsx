    ...
    <div className="space-y-6 league-fade-in">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-ufa-border bg-ufa-panel p-4">
          <h3 className="font-semibold text-ufa-text text-sm mb-3">{t.clubPulse}</h3>
          <div className="divide-y divide-ufa-border">
            {inboxUnread > 0 && (
              <button type="button" onClick={() => onNavigate('inbox')} className="ufa-action-row">
                <span><strong>{t.clubPulseInbox(inboxUnread)}</strong><small>{t.openInbox}</small></span>
                <span aria-hidden="true">→</span>
              </button>
            )}
            {transferWindow.open && (
              <button type="button" onClick={() => onNavigate('club-transfers')} className="ufa-action-row">
                <span><strong>{t.clubPulseWindow}</strong><small>{t.openTransfers}</small></span>
                <span aria-hidden="true">→</span>
              </button>
            )}
            {playerFix && playerFix.status !== 'completed' && (
              <button type="button" onClick={() => onNavigate('match')} className="ufa-action-row">
                <span><strong>{t.clubPulseMatch}</strong><small>{t.goToMatch}</small></span>
                <span aria-hidden="true">→</span>
              </button>
            )}
            {inboxUnread === 0 && !transferWindow.open && (!playerFix || playerFix.status === 'completed') && (
              <button type="button" onClick={() => onNavigate('ultiworld')} className="ufa-action-row">
                <span><strong>{t.clubPulseNews}</strong><small>{t.newsBrand}</small></span>
                <span aria-hidden="true">→</span>
              </button>
            )}
          </div>
