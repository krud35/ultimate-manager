        ...
        {keys.map((key) => {
          ...
          return (
            <li key={key} className="flex flex-col gap-0.5 sm:flex-row sm:items-center sm:gap-3">
              <span className="text-xs text-ufa-muted sm:w-[9.5rem] shrink-0" title={lang === 'pl' ? ATTRIBUTE_HELP_PL[key]?.[1] : undefined}>
                {lang === 'pl' ? (ATTRIBUTE_HELP_PL[key]?.[0] ?? labels[key] ?? key) : (labels[key] ?? key)}
              </span>
              {display.kind === 'exact' ? (
        ...
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-ufa-border bg-ufa-panel/95 px-5 py-4 backdrop-blur">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              {Number.isFinite(player.body?.heightCm) && (
                <span className="text-xs text-ufa-muted">{lang === 'pl' ? 'Wzrost' : 'Height'}: {player.body.heightCm} cm</span>
              )}
              {Number.isFinite(player.body?.massKg) && (
                <span className="text-xs text-ufa-muted">{lang === 'pl' ? 'Masa' : 'Mass'}: {player.body.massKg} kg</span>
              )}
              {Number.isFinite(player.body?.armSpanCm) && (
                <span className="text-xs text-ufa-muted">{lang === 'pl' ? 'Rozpiętość ramion' : 'Arm span'}: {player.body.armSpanCm} cm</span>
              )}
              {isOwnPlayer && (
                <span
