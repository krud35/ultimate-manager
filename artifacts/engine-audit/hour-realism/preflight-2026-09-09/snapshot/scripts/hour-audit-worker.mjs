...
function teams() {
  ...
  home.tactics = tactics(home, job.homePatch)
  away.tactics = tactics(away, job.awayPatch)
...
THROW_SCAN_DIAGNOSTICS.observe = job.observe === false ? null : row => {
  ...
  if (scans.length < 80) scans.push(row)
  else if (totalScans % 20 === 0) { scans.splice(40, 1); scans.push(row) }
}
  ...
  for (const e of events) {
    ...
    if ((e.type === 'throw_success' || e.type === 'throw_fail') && last && job.observe !== false) {
      ...
      if (!previous || rank < previous.rank) {
        ...
        candidates.set(category, { category, rank, jobId: job.id, seed: job.seed, pointIndex,
          selectedAction: index, outcome: e.type, clips, scans: scans.filter(x => x.throwerId === last.throwerId),
...
function flushCandidates() {
  for (const [cat, c] of candidates) {
    ...
    result.replayCandidates.push({ category: cat, rank: c.rank, path: filename, jobId: job.id, pointIndex: c.pointIndex })
  }
}
...
try {
  if (job.kind === 'setups' || job.kind === 'effects') {
    const rows = []
    for (const spec of job.cases) for (const variant of job.variants ?? [null]) {
      ...
      append('situations.jsonl', { jobId: job.id, ...rows.at(-1) })
    }
    ...
    if (rotate) for (const side of ['home', 'away']) session[side].tactics = adaptive
    ...
    do {
      ...
      playNextPoint(session, {}, { rotateHome: rotate, rotateAway: rotate, aiHome: adaptive, aiAway: adaptive, fastMode: job.fast === true })
