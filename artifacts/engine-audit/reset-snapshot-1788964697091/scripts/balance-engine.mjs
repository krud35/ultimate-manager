...
import { RESET_PASS_CALIBRATION } from '../src/matchEngine/ai/throwerBrain.js'
...
if (process.argv.includes('--safe-resets')) RESET_PASS_CALIBRATION.enabled = true
if (process.argv.includes('--legacy-reset-pass')) RESET_PASS_CALIBRATION.enabled = false
if (process.argv.includes('--purposeful-layout')) FLIGHT_APPROACH_CALIBRATION.purposefulLayout = true
if (process.argv.includes('--legacy-layout')) FLIGHT_APPROACH_CALIBRATION.purposefulLayout = false
for (const [flag, config, key] of [
  ...
  ['--reset-speed-reserve', RESET_PASS_CALIBRATION, 'speedReserve'],
  ['--reset-uncertainty', RESET_PASS_CALIBRATION, 'maxUncertaintyM'],
  ['--reset-late', RESET_PASS_CALIBRATION, 'lateAllowanceMs'],
]) {
