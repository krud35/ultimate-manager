...
export function claimAwarenessFor(player) {
  if (!player) return 1
  const read =
    subStat(player, 'mental', 'spatialAwareness') * 0.5 +
 ...
 */
export function threatCellForMark(mark, cells, { speed = 7, player = null, role = 'defense', rng = null } = {}) {
  if (!mark || !cells?.length) return null
  const maxAhead = YARD_REF_M
  let best = null
  let bestThreat = -Infinity
  const err = perceptionError(player, role, rng)
  let bestFreeness = 0
  for (const cell of cells) {
    ...
    // Ten sam szum i ta sama kolejność losowań co perceiveSpaceMap, bez tablicy
    // kopii wszystkich komórek. Materializujemy wyłącznie wybraną komórkę.
    const freeness = err ? Math.max(0, Math.min(1, cell.freeness + (rng.float() - 0.5) * PERCEPTION_NOISE * err)) : cell.freeness
    const threat = freeness * reach * value
    if (threat > bestThreat) {
      bestThreat = threat
      best = cell
      bestFreeness = freeness
    }
  }
  return best ? { ...best, freeness: bestFreeness, threat: bestThreat } : null
}
 ...
 * atak czyta `spatialAwareness` + `offensiveSystemsKnowledge`, obrona `spatialAwareness` +
...
function perceptionError(player, role, rng) {
  if (!player || !rng?.float) return 0
  const systems =
    role === 'defense'
      ? subStat(player, 'defensive', 'defensiveSystemsKnowledge')
      : subStat(player, 'offensive', 'offensiveSystemsKnowledge')
  const read = subStat(player, 'mental', 'spatialAwareness') * 0.5 + systems * 0.5
  const err = Math.max(0, Math.min(1, 1 - (read - 50) / 45))
  return err < 0.02 ? 0 : err
}
