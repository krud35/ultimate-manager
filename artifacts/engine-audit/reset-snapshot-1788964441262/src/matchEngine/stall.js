...
/** Liczenie markera jest osobne od czasu posiadania. „Jeden” pada na początku;
 * „dziesięć” po dziewięciu pełnych odstępach. Wyjście poza 3 m lub zmiana
 * markera zeruje liczenie (WFDF 9.3–9.4). Brak markera nie powoduje stall-outu.
 */
export function advanceStallClock(clock, marker, thrower, dtMs) {
  const id = marker?.id ?? marker?.player?.id ?? null
  const legal = id != null && thrower && Math.hypot(marker.x - thrower.x, marker.y - thrower.y) <= 3
  if (!legal) { clock.markerId = null; clock.elapsedMs = 0; return 0 }
  if (clock.markerId !== id) { clock.markerId = id; clock.elapsedMs = 0 }
  else clock.elapsedMs += Math.max(0, dtMs)
  return activeStallCount(clock)
}

export function activeStallCount(clock) {
  return clock?.markerId == null ? 0 : Math.min(STALL_MAX, 1 + Math.floor((clock.elapsedMs ?? 0) / STALL_SECOND_MS))
}
