...
import { applyMotionTraceToTracks, sampleDiscFromMotionTrace, sampleStateFromMotionTrace, resolveCatchPointFromMotionTrace, resolveTurnoverPointFromMotionTrace, lockPlayerTrackToCatchAnchor } from './motionFromTicks.js'
...
export function buildThrowActionClip(
  ...
  if (attemptEv.motionTrace?.frames?.length) {
    const preservePositions = attemptEv.motionTrace.preservePositions === true
    applyMotionTraceToTracks(tracks, attemptEv.motionTrace, tacticalAll, {
      setupMs,
      releaseMs,
      flightMs,
    }, { anchorPivot: preservePositions ? {} : anchorPivot })
    if (!preservePositions && anchorPivot[attemptEv.throwerId]) {
    ...
    if (stallMarkerId != null && !preservePositions) {
...
export function buildFieldActionClip(
  ...
  if (ev?.type === 'stall_pressure') {
    ...
    if (stallMarkerId != null && ev.throwerId != null && !ev.motionTrace?.preservePositions) {
      syncStallMarkerTrackToThrower(
        tracks,
        stallMarkerId,
        ev.throwerId,
...
export function sampleFieldActionClip(clip, elapsedMs) {
  ...
  // Nowy trace opisuje również podniesienie dysku i legalne liczenie. Nie zastępuj
  // ich syntetycznym dyskiem w dłoni ani zegarem posiadania (stare zapisy: fallback).
  ...
  if (tracedState?.disc && (phase !== 'flight' || tracedState.disc.state === DISC_STATE.ON_GROUND)) {
    ...
    discState = tracedState.disc.state
  }
  ...
  const holdBasedStall =
    tracedState?.stallCount != null ? tracedState.stallCount : clip.holdStartMs != null
  ...
  const sampledMarkerId = tracedState?.markerId ?? clip.markerId
  if (tracedState?.players) {
    const support = new Map(tracedState.players.map(p => [p.id, p.toeInContact]))
    players = players.map(p => ({ ...p, toeInContact: support.get(p.id) ?? null }))
  }
  if (sampledMarkerId != null) {
    for (const p of players) {
      if (p.id === sampledMarkerId) {
        p.role = 'marker'
        p.isActiveMark = true
