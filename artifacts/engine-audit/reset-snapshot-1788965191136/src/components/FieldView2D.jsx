...
const RUNOFF_M = 12
const FULL_VIEW = `${-RUNOFF_M} ${-RUNOFF_M} ${LENGTH_M + 2 * RUNOFF_M} ${WIDTH_M + 2 * RUNOFF_M}`
...
function meterToPct(x, y) {
  return {
    xPct: ((x + RUNOFF_M) / (LENGTH_M + 2 * RUNOFF_M)) * 100,
    yPct: ((y + RUNOFF_M) / (WIDTH_M + 2 * RUNOFF_M)) * 100,
  }
}
    ...
    <div className={`field-view-2d ${className}`}>
      ...
      <div className="field-view-2d__stage">
        <svg
          ...
          aria-hidden
        >
          <rect x={-RUNOFF_M} y={-RUNOFF_M} width={LENGTH_M + 2 * RUNOFF_M} height={WIDTH_M + 2 * RUNOFF_M} fill="#173b30" />
          ...
          <rect className="field-line" x={0} y={0} width={LENGTH_M} height={WIDTH_M} fill="none" />
          ...
          {frame.players.map((p) => {
            ...
            return (
              <g key={`${fieldPointKey ?? 'p'}-${p.teamId}-${p.id}`}>
                ...
                {p.toeInContact && (
                  <g>
                    <line x1={pt.x} y1={pt.y} x2={p.toeInContact.x} y2={p.toeInContact.y} stroke="#fde047" strokeWidth={0.18} />
                    <circle cx={p.toeInContact.x} cy={p.toeInContact.y} r={0.22} fill="#fde047"><title>Toe-in</title></circle>
                  </g>
                )}
              </g>
            )
