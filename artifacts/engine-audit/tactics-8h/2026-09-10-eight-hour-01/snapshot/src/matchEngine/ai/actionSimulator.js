...
import { scanThrowOptions, separationFromSituation, THROW_SCAN_DIAGNOSTICS } from './throwerBrain.js'
...
import { discPositionHeld, discPositionInFlight, discPositionOnGround } from '../discState.js'
...
import { looseDiscLanding } from './discTrajectory.js'
import { firstDiscContact, agentAtContact } from './discContact.js'
import {
...
import { mergeTraitAndCoachMods, teamStackDepthBiasM } from '../coachDirectives.js'
import {
  STALL_MAX,
  advanceStallClock,
  activeStallCount,
...
import { resetPlayerPerception } from './playerPerception.js'
import { coordinateSwitches } from './teamCoordination.js'
import { eligibleReceiver, groundedInBounds, advanceCatchLanding, catchScores, attemptToeIn, pointInBounds, boundaryRestart } from './catchRules.js'
import { makeDeflection } from './discDeflection.js'
import { diagnoseThrow } from './throwDiagnosis.js'
 ...
 *
 * HISTORYCZNE, wcześniejszy model kontaktu (tmp-laneblock-sweep.mjs, 8 meczów/arm):
 *   base/span      completion  bloki/mecz (na torze)  hold%   huck
 *   0.16 / 0.42      92.6%      13.4  (4.4)          68.5%  79.3%
 *   0.10 / 0.28      94.0%       9.4  (2.1)          74.2%  83.3%
 *   0.06 / 0.18      94.1%       8.6  (1.9)          72.6%  82.7%
 *   0.03 / 0.12      93.7%       7.9  (0.9)          68.9%  85.5%
 *
 * W tamtym pomiarze bloki na torze to ~22% wszystkich bloków — reszta zostaje na odbiorcy (nie dobiegł
 ...
 * Aktualna kalibracja: artifacts/engine-audit/COMPLETION-90-93.md.
 */
...
/** Starsza ocena zbiorczego okna, pozostawiona wyłącznie do porównań diagnostycznych.
 * Bieżąca symulacja rozstrzyga kontakt w kolejności czasu, poniżej.
 */
export function computeGeometricResolution(shadowContest, flight, rng = null) {
...
function snapshotFrame(ms, offenseAgents, defenseAgents, throwerId, disc = null, markerId = null, stallCount = null) {
  const players = [
    ...offenseAgents.map((a) => ({
      ...
      diveHeading: a.diveHeading ?? 0,
      toeInContact: a.toeInContact ?? null,
    ...
    ...defenseAgents.map((a) => {
      ...
      return {
        ...
        ...(THROW_SCAN_DIAGNOSTICS.observe ? { audit: { targetX: a.targetX, targetY: a.targetY, stateMs: a.stateMs } } : {}),
        layout: a.layout ?? false,
        diving: a.diving ?? false,
        diveHeading: a.diveHeading ?? 0,
...
export function runContinuousThrowSimulation({
  ...
  const offenseLayout = layoutPlayersOnField(
    ...
    true,
    {
      ...
      attackSign,
      discYMeters: discY,
      // Głębokość stacka z założeń trenerskich wchodzi już w ustawienie POCZĄTKOWE —
      // sam bias w celu dryfu (cutterBrain) nie zdążał się zrealizować w jednej akcji.
      stackDepthBiasM: teamStackDepthBiasM(offenseTeam?.tactics),
    },
  )
  ...
  let offenseAgents = layoutToAgents(
    ...
    if (a.isThrower) {
      return {
        ...a,
        x: pickupPending && carriesRole ? seed.x : discX,
        y: pickupPending && carriesRole ? seed.y : discY,
        state: CUTTER_STATE.WAITING,
        vx: pickupPending && carriesRole ? seed.vx ?? 0 : 0,
        vy: pickupPending && carriesRole ? seed.vy ?? 0 : 0,
        player: a.player,
        subRole: a.subRole,
      }
    }
  ...
  if (personMark && personMatchups instanceof Map) {
    for (const off of offenseAgents) {
      ...
      if ((off.isThrower || off.player?.id === thrower.id) && !carriesDefense) {
  ...
  if (initialFlightState) {
    offenseAgents = initialFlightState.offenseAgents
    defenseAgents = initialFlightState.defenseAgents
  }
  let throwerAgent = offenseAgents.find((a) => a.isThrower)
  let pickupEndMs = pickupPending ? null : 0
  ...
  const stallClock = { markerId: null, elapsedMs: 0, ...startStallClock }
  ...
  /** Krótka przerwa przed kolejnym dotknięciem tego samego zawodnika. */
  const contactTried = new Map()
  let pendingTouch = null
  let pendingCatch = null
  for (const agent of [...offenseAgents, ...defenseAgents]) resetPlayerPerception(agent.player)
  ...
  for (let tick = initialFlightState ? 1 : 0; tick < resolvedMaxTicks; tick += 1) {
    ...
    for (const a of [...offenseAgents, ...defenseAgents]) {
      if ((a.z ?? 0) <= 0) a.lastGroundInBounds = groundedInBounds(a)
    }
    if (!flight && ms % 500 === 0 && personMark) coordinateSwitches(defenseAgents, offenseAgents, personMatchups, ms)
    if (pendingCatch) {
      const interception = pendingCatch.securedInterception
      const catchingTeam = interception ? (possessionTeam === 'home' ? 'away' : 'home') : possessionTeam
      const advance = a => a.id === pendingCatch.receiverId ? advanceCatchLanding(a, DT_SEC, catchingTeam) : brake(a)
      const brake = a => ({ ...a, ...integrateAgentMotion(a, a.x, a.y, Math.hypot(a.vx ?? 0, a.vy ?? 0), DT_SEC, true) })
      ...
      if (catcher.z <= 0) {
        ...
        const landingDrop = (catcher.layout ? 0.08 : 0.02) * (1 - subStat(catcher.player, 'offensive', 'catching') / 100)
        if ((contactResolution.success || contactResolution.securedInterception) && rng.float() < landingDrop) {
          contactResolution = { ...pendingCatch, success: false, securedInterception: false, isDrop: !interception, reason: interception ? 'lane_block' : 'landing_drop' }
        }
        contactResolution.landing = { x: catcher.x, y: catcher.y,
          ms: pendingCatch.firstLandingMs ?? ms - SIM_TICK_MS + (catcher.landingFraction ?? 1) * SIM_TICK_MS }
        contactResolution.toeIn = catcher.toeInContact ?? null
        contactResolution.pivotPoint = catcher.toeInContact ?? { x: catcher.x, y: catcher.y }
        contactResolution.scoresPoint = contactResolution.success && catchScores(catcher, possessionTeam)
        contactResolution.interceptionScoresPoint = contactResolution.securedInterception && catchScores(catcher, catchingTeam)
        break
      }
      continue
    }
    const holdMs = holdStartMs + Math.max(0, ms - (pickupEndMs ?? ms))
    holdMsAtEnd = holdMs
    const liveStall = flight ? activeStallCount(stallClock) : advanceStallClock(
    ...
    const offensePositions = offenseAgents.map((a) => ({
      ...
      x: a.x,
      y: a.y,
      // Przewidywanie przecięcia tras potrzebuje ruchu kolegów, nie tylko ich pozycji.
      vx: a.vx ?? 0,
      vy: a.vy ?? 0,
    ...
    if (flight) {
      flight.sightBlockers = [...offenseAgents, ...defenseAgents]
      ...
      if (flight.deflection && flight.elapsedMs >= (flight.recoveryNextMs ?? 0)) {
        flight.recoveryNextMs = flight.elapsedMs + 200
        const choices = offenseAgents.filter(a => a.id !== flight.throwerId).map(a => ({ agent: a,
          target: interceptForAgent(flight, a, a.player, 'offense', sprintSpeedMps(a.player)) }))
          .filter(c => c.target.hasObservedDeflection && c.target.reachable && c.target.legal)
          .sort((a, b) => a.target.atMs - b.target.atMs)
        const incumbent = choices.find(c => c.agent.id === (flight.recoveryReceiverId ?? flight.receiverId))
        const best = choices[0]
        if (best && (!incumbent || best.target.atMs + 250 < incumbent.target.atMs)) {
          flight.recoveryReceiverId = best.agent.id
        }
      }
      ...
      defenseAgents = defenseAgents.map((defAgent) => {
        ...
        // więc słabszy obrońca przegapi okazję, którą lepszy wykorzysta.
        {
          ...
          if (!globalThis.__OFF_FLIGHTPOACH && recvAgentNow && !defAgent.poachCommitted) {
            const myTarget = interceptForAgent(flight, defAgent, dp, 'defense', sprintSpeedMps(dp, 'defense'))
            const rp = recvAgentNow.player ?? recvAgentNow
            const hisTarget = interceptForAgent(flight, recvAgentNow, rp, 'offense', sprintSpeedMps(rp, 'offense'))
            const canBeatHim = myTarget.reachable && (!hisTarget.reachable
              || myTarget.atMs + POACH_BEAT_MARGIN_SEC * 1000 < hisTarget.atMs)
            if (canBeatHim) {
              const read =
          ...
          if (defAgent.poachCommitted) {
            const contested = tickFlightContestAgent(
              defAgent, intercept, dp, 'defense', discSample, rng, 1, flight,
            )
      ...
      // Kontakty z tego samego kroku: najpierw wcześniejsze przecięcie toru,
      // niezależnie od tego, czy dotyczy odbiorcy czy obrońcy.
      const previousFrame = frames.at(-1)
      const previousDisc = prevDiscSample ?? discSample
      const candidates = []
      for (const agent of [...defenseAgents, ...offenseAgents.filter(a => a.id !== flight.throwerId)]) {
        const id = agentPlayerId(agent)
        if (flight.elapsedMs < (contactTried.get(id) ?? -Infinity) || (flight.touches?.length ?? 0) >= 8) continue
        const isReceiver = offenseAgents.some(a => a.id === id)
        const player = agent.player ?? agent
        if (LANE_BLOCK_CALIBRATION.deflectionReaction && flight.deflection
          && flight.elapsedMs - flight.deflection.atMs < Math.max(80, 200 - subStat(player, 'mental', 'reactions'))) continue
        let profile = contactProfiles.get(id)
        if (!profile) {
          profile = { standing: standingReachM(player), horizontal: horizontalReachM(player),
            reactionMs: isReceiver ? 0 : defenderReactionDelayMs(player) }
          contactProfiles.set(id, profile)
        }
        if (flight.elapsedMs < profile.reactionMs) continue
        const before = previousFrame?.players.find(p => p.id === id) ?? agent
        const contact = firstDiscContact(previousDisc, discSample, before, agent, profile)
        if (contact && contact.z > 0) {
          const atContact = agentAtContact(before, agent, contact.fraction)
          const support = isReceiver ? attemptToeIn(atContact, possessionTeam) : atContact
          candidates.push({ ...contact, id, player, isReceiver, agent: support,
            outOfBounds: isReceiver && !eligibleReceiver(support), tie: rng.float() })
        }
      }
      candidates.sort((a, b) => a.fraction - b.fraction || a.tie - b.tie)
      const discSpeedMps = Math.hypot(discSample.x - previousDisc.x,
        discSample.y - previousDisc.y, (discSample.z ?? 0) - (previousDisc.z ?? 0)) / DT_SEC
      for (const c of candidates) {
        // Pressure exists when a reacted defender can reach this contact point,
        // even if their swept entry happened in a neighbouring tick.
        const contested = !LANE_BLOCK_CALIBRATION.spatialPressure
          ? candidates.some(d => !d.isReceiver && Math.abs(d.fraction - c.fraction) < 0.1)
          : c.isReceiver && defenseAgents.some(d => {
          const id = agentPlayerId(d), profile = contactProfiles.get(id)
          if (!profile || flight.elapsedMs < profile.reactionMs) return false
          const before = previousFrame?.players.find(p => p.id === id) ?? d
          const at = agentAtContact(before, d, c.fraction)
          return !!firstDiscContact(c, c, at, at, profile)
        })
        const canWait = c.isReceiver
          ? !contested && !defenseAgents.some(d => Math.hypot(d.x - c.x, d.y - c.y) < 2.5)
          : LANE_BLOCK_CALIBRATION.selectContact && !candidates.some(d => d.isReceiver && !d.outOfBounds)
        if (!c.outOfBounds && c.envelope > 0.55 && canWait) {
          // Nie chwytaj odruchowo końcami palców, gdy dysk zmierza w ciało.
          // Pod presją bierzemy najwcześniejszą możliwość; bez presji można poczekać.
          const futureDisc = sampleFlightDisc(flight, flight.elapsedMs + SIM_TICK_MS)
          const futureAgent = { ...c.agent, x: c.agent.x + (c.agent.vx ?? 0) * DT_SEC,
            y: c.agent.y + (c.agent.vy ?? 0) * DT_SEC }
          const future = firstDiscContact(discSample, futureDisc, c.agent, futureAgent, contactProfiles.get(c.id))
          if (futureDisc.z > 0.15 && future && future.envelope < c.envelope - 0.01) continue
        }
        contactTried.set(c.id, flight.elapsedMs + 160)
        const chance = c.isReceiver ? catchSuccessChance(c.player, {
          discZ: c.z, contested, layoutAttempt: !!c.agent.layout,
          catchBonus: playerMatchMods(c.player).catchBonus ?? 0, reachStrain: c.envelope,
        }) : laneBlockChance(c.player, c.envelope, discSpeedMps)
        if (c.outOfBounds) {
          contactResolution = { success: false, isBlock: false, isDrop: false, reason: 'out_of_bounds' }
        } else if (rng.float() > chance) {
          if (!c.isReceiver) continue
          contactResolution = { success: false, isBlock: false, isDrop: true,
            defenderId: null, reason: 'drop' }
        } else {
          contactResolution = { success: c.isReceiver, isBlock: !c.isReceiver, isDrop: false,
            defenderId: c.isReceiver ? null : c.id, reason: c.isReceiver ? 'catch' : 'lane_block' }
          // A defender may secure a catch after making contact, rather than
          // every successful play automatically becoming a tip back to offense.
          if (!c.isReceiver && LANE_BLOCK_CALIBRATION.interceptions && eligibleReceiver(c.agent)
            && !c.agent.diving && discSpeedMps < 14
            && rng.float() < catchSuccessChance(c.player, { discZ: c.z, reachStrain: c.envelope, contested: true })) {
            contactResolution.securedInterception = true
            contactResolution.reason = 'interception'
            contactResolution.receiverId = c.id
            contactResolution.interceptionScoresPoint = (c.agent.z ?? 0) <= 0
              && catchScores(c.agent, possessionTeam === 'home' ? 'away' : 'home')
          }
        }
        if (c.isReceiver) contactResolution.receiverId = c.id
        if (contactResolution.success && (c.agent.z ?? 0) <= 0) contactResolution.scoresPoint = catchScores(c.agent, possessionTeam)
        if (contactResolution.success && (c.agent.z ?? 0) <= 0) {
          contactResolution.toeIn = c.agent.toeInContact ?? null
          contactResolution.pivotPoint = c.agent.toeInContact ?? { x: c.agent.x, y: c.agent.y }
          if (c.agent.toeInContact) {
            const catcher = offenseAgents.find(a => a.id === c.id)
            catcher.toeInContact = c.agent.toeInContact
          }
        }
        contactResolution.intendedReceiverId = flight.receiverId
        const contactMs = ms - SIM_TICK_MS * (1 - c.fraction)
        // Ślad pozostaje na siatce 20 ms; dokładniejszy moment i punkt kontaktu są metadanymi.
        contactResolution.contact = { x: c.x, y: c.y, z: c.z, ms: contactMs, playerId: c.id }
        if (!c.isReceiver) contactResolution.laneBlock = { ...contactResolution.contact,
          defenderId: c.id, envelope: c.envelope, discSpeedMps }
        flight.touches ??= []
        flight.touches.push({ ...contactResolution.contact, type: contactResolution.reason,
          inBounds: eligibleReceiver(c.agent) })
        if (!contactResolution.success && !contactResolution.securedInterception && !c.outOfBounds) {
          pendingTouch = contactResolution
          flight.deflection = makeDeflection(c, { x: (discSample.x - previousDisc.x) / DT_SEC,
            y: (discSample.y - previousDisc.y) / DT_SEC, z: (discSample.z - previousDisc.z) / DT_SEC },
          flight.elapsedMs - SIM_TICK_MS * (1 - c.fraction), !c.isReceiver)
          flight.recoveryNextMs = 0
          contactResolution = null
        } else if ((contactResolution?.success || contactResolution?.securedInterception) && (c.agent.z ?? 0) > 0) {
          pendingCatch = contactResolution
          const remainingSec = DT_SEC * (1 - c.fraction)
          const caught = advanceCatchLanding(c.agent, remainingSec, pendingCatch.securedInterception
            ? (possessionTeam === 'home' ? 'away' : 'home') : possessionTeam)
          if (caught.z <= 0) pendingCatch.firstLandingMs = contactMs + remainingSec * (caught.landingFraction ?? 1) * 1000
          if (pendingCatch.securedInterception) defenseAgents = defenseAgents.map(a => a.id === c.id ? caught : a)
          else offenseAgents = offenseAgents.map(a => a.id === c.id ? caught : a)
          contactResolution = null
        }
        discSample = flight.deflection && !contactResolution && !pendingCatch
          ? sampleFlightDisc(flight, flight.elapsedMs)
          : { ...discSample, x: c.x, y: c.y, z: c.z }
        break
      }
      ...
      if (discSample.timeToDisc <= AERIAL_WINDOW_MS) {
        ...
        // Dawne okno pozostaje do diagnostyki; nie rozstrzyga już wyniku.
        ...
        if (recvAgent) {
          if (inContestWindow) {
            ...
            if (d3 < shadowContest.receiverMinDist3D) {
              ...
              shadowContest.receiverClosestMs = ms
      ...
      offenseAgents = offenseAgents.map((agent, idx) => {
        if (agent.isThrower) {
          if (pickupPending) {
            const distance = Math.hypot(agent.x - discX, agent.y - discY)
            if (distance <= 0.45 && Math.hypot(agent.vx ?? 0, agent.vy ?? 0) < 1) {
              pickupPending = false
              pickupEndMs = ms
              scanCache = null
              scanCacheMs = -1e9
              return { ...agent, x: discX, y: discY, vx: 0, vy: 0 }
            }
            return { ...agent, ...integrateAgentMotion(agent, discX, discY,
              Math.min(sprintSpeedMps(agent.player), Math.max(0.2, distance * 3)), DT_SEC, true, 'offense') }
          }
    ...
    if (flight) {
      ...
      discSnapshot = discPositionOnGround(discX, discY)
    } else if (throwerAgent) {
      discSnapshot = discPositionHeld(throwerAgent.x, throwerAgent.y, attackSign)
    }

    if (behaviorBoxScore) {
      const clock = ms - (pickupEndMs ?? 0)
      const fakeWindow = !flight && !pickupPending ? throwingFakePhase(clock, 1) : 0
      const fakePhase = !flight && !pickupPending ? throwingFakePhase(clock, playerMatchMods(thrower).fakeFrequency) : 0
      observeStyleTick(behaviorBoxScore, behaviorBefore, offenseAgents, defenseAgents, {
        dtSec:DT_SEC, throwerId:pickupPending ? null : thrower.id, disc:{...discSnapshot,inFlight:!!flight},
    ...
    if (!flight && !pickupPending) {
      ...
      if (option && !throwingFakePhase(ms - (pickupEndMs ?? 0), throwerCoach.fakeFrequency) && ms - (pickupEndMs ?? 0) >= Math.max(0, releaseGateMs)) {
        ...
        if (scanCacheMs !== ms) { scanCache = null; scanCacheMs = -1e9; continue }
        if (globalThis.__DEC) {
        ...
        throwDecision = {
          ...
          plannedShape: option.shape ? { ...option.shape, plannedArrival: option.plannedArrival,
            validationStatus: option.validationStatus, decisionAtMs: ms, releaseAtMs: ms,
            observationAtMs: ms - (option.agent?.observationAgeMs ?? 0),
            receiverObservation: { x: recvAgent.x, y: recvAgent.y, vx: recvAgent.vx ?? 0, vy: recvAgent.vy ?? 0 } } : null,
        ...
        if (onThrowCommitted) {
          ...
          flight = createFlightContext({
            ...
            plannedShape: throwDecision.plannedShape,
  ...
  // Brak kontaktu nie jest blokiem: samo zbliżenie obrońcy nie wystarcza.
  const geoResolution = contactResolution ?? pendingTouch ?? { success: false, isBlock: false,
    isDrop: false, defenderId: null, reason: 'not_in_reach' }

  // Dopiero geometria ustala stan końcowy. Wcześniejszy resolveThrow nie może
  // ani przestawiać zawodników, ani przyklejać dysku do odbiorcy.
  if (geoResolution && frames.length) {
    if (geoResolution.success || geoResolution.securedInterception) {
      ...
      const velocity = { x: (start.x - (previous?.disc?.x ?? start.x)) / dt * factor,
        y: (start.y - (previous?.disc?.y ?? start.y)) / dt * factor,
        z: ((start.z ?? 0) - (previous?.disc?.z ?? start.z ?? 0)) / dt }
      const durationMs = looseDiscLanding(start, velocity, 0).durationSec * 1000
      // Zawodnicy hamują w czasie spadania; żadnego przestawiania pod wynik.
      for (let elapsed = SIM_TICK_MS; elapsed < durationMs + SIM_TICK_MS; elapsed += SIM_TICK_MS) {
        const landing = looseDiscLanding(start, velocity, elapsed / 1000)
        const brake = a => ({ ...a, ...integrateAgentMotion(a, a.x, a.y,
    ...
    flight.resolution = geoResolution
  }
  if (!geoResolution.success && !geoResolution.securedInterception) {
    const ground = frames.at(-1)?.disc
    if (geoResolution.reason === 'out_of_bounds' || (ground && !pointInBounds(ground))) {
  ...
  return {
    ...
    commitAbort: false,
    commitMeta,
    receiver: offenseAgents.find(a => a.id === geoResolution.receiverId)?.player ?? throwDecision.receiver,
    intendedReceiverId: throwDecision.receiver.id,
    ...
    receiverSubRole: offenseAgents.find(a => a.id === geoResolution.receiverId)?.subRole
      ?? throwDecision.receiverSubRole ?? null,
