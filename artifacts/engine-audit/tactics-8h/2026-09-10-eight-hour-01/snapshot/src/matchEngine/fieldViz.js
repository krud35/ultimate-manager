...
export function layoutPlayersOnField(
  ...
  const {
    ...
    defenseTactics = null,
    /**
     * Głębokość stacka z założeń trenerskich (coachDirectives.stackDepth), w metrach
     * wzdłuż osi ataku. Musi wejść już TUTAJ, w ustawienie początkowe: sam bias w celu
     * dryfu (cutterBrain) dawał przesunięcie 0.43 m przy komendzie 4.2 m, bo zawodnicy
     * dojeżdżają na slot truchtem i nie zdążają w trakcie jednej akcji. Ustawieni od
     * razu tam, gdzie mają stać, utrzymują to potem tym samym biasem w dryfie.
     */
    stackDepthBiasM = 0,
  } = layoutOptions
  ...
  if (isOffense) {
    ...
    // Rzucający zostaje na miejscu — stoi przy dysku, nie w stacku; reset także, bo jego
    // pozycję wyznacza dysk (tak samo pomija go applySlotDepthBias w cutterBrain).
    if (!stackDepthBiasM) return offense
    return offense.map((p) =>
      p.id === throwerId || p.fieldRole === 'dump'
        ? p
        : { ...p, x: clampFieldX(p.x + attackSign * stackDepthBiasM) },
    )
  }
