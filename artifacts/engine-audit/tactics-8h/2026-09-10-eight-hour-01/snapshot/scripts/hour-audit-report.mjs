...
function sum(rows) {
  ...
  for (const row of rows) for (const [k, v] of Object.entries(row.stats ?? {})) {
    if (typeof v === 'number') out[k] = k === 'maxSpeed' ? Math.max(out[k] ?? 0, v) : (out[k] ?? 0) + v
    else if (k === 'types') {
      out.types ??= {}
...
export function writeHourReport(out) {
  ...
  clips.push(...candidates.filter(c => c.category === 'alarm').sort((a, b) => b.severity - a.severity || a.rank.localeCompare(b.rank)).slice(0, 24))
  const problems = []
  ...
  fs.writeFileSync(path.join(out, 'situations.jsonl'), '')
  for (const file of fs.readdirSync(path.join(out, 'checkpoints')).filter(f => f.endsWith('-situations.jsonl')))
    fs.appendFileSync(path.join(out, 'situations.jsonl'), fs.readFileSync(path.join(out, 'checkpoints', file)))
