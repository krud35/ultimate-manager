...
if (args.includes('--prepare')) {
  ...
  const preflight = smoke ? null : JSON.parse(fs.readFileSync(path.resolve(arg('--preflight', 'MISSING_PREFLIGHT'), 'ACCEPTANCE.json')))
  if (!smoke) {
    if (preflight.status !== 'PASS') throw new Error('Preflight did not pass')
    for (const [f, h] of Object.entries(preflight.hashes)) if (hash(fs.readFileSync(path.join(root, 'scripts', f))) !== h) throw new Error(`Preflight stale: ${f}`)
  }
  ...
  save('manifest.json', { schema: 1, status: 'PREPARED', preparedAt: new Date().toISOString(), smoke, preflight,
  ...
  try {
    ...
    ticker = setInterval(() => {
      control()
      // Keep the large frozen queues out of the periodic write path.
      save('progress.json', { status: m.status, phase: m.phase, elapsedMs: elapsed(), updatedAt: new Date().toISOString(),
        completed: completed.size, activeWorkers: [...active.entries()].map(([id, c]) => ({ id, pid: c.pid })) })
    }, 5000)
    ...
    for (const phase of m.phases) {
      if (requested) break
      if (resume && m.resumeFromPhase && m.phases.findIndex(p => p.id === phase.id) < m.phases.findIndex(p => p.id === m.resumeFromPhase)) continue
      if (phase.end * 60000 <= before) continue
      m.phase = phase.id; persist()
      if (!m.smoke && phase.id === 'validation' && !m.selectionDevelopment) {
        const selection = selectConfigs(readResults(out, 'development'), candidates, 16, 2)
      ...
      if (!m.smoke && phase.id === 'holdout' && !m.selectionFinal) {
        const selection = selectConfigs(readResults(out, 'validation'), m.selectionDevelopment ?? [], 4, 1)
      ...
      const bytes = list(out).reduce((n, file) => n + fs.statSync(file).size, 0)
      if (bytes > m.diskBudgetBytes) stop('DISK_BUDGET')
    ...
    m.reportFinishedAt = new Date().toISOString(); persist()
