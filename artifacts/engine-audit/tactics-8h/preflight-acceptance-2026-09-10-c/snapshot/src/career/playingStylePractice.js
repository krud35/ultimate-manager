...
const RELEVANT_INSTRUCTIONS = {
 huck_lover:['throw_hucks'], dump_guy:['dump_first','no_hucks'],
 good_insides:['break_mark'], under_cutter:['cut_under'], deep_threat:['cut_deep'],
}
...
export function evaluateStyleMatch(match, style) {
 ...
 for(const mode of Object.values(match.evidence?.modes ?? {})) {
  ...
  if(rule.throwTypes || rule.curve) {
   ...
   for(const [key,row] of Object.entries(mode.throwResults ?? {})) {
    const [type,technique,curve]=key.split('|')
    if(rule.curve && (!curve || curve==='unobserved' || curve==='unknown'))continue
    if(rule.technical && !['forehand','backhand'].includes(technique))continue
    d+=row.attempts
 ...
 if(rule.inverse && n<=0)return null
 const ratio=n/d
 ...
 const instructed=(match.evidence?.instructionSnapshots??[]).some(snapshot=>
  Array.isArray(snapshot.individual) && snapshot.individual.some(id=>RELEVANT_INSTRUCTIONS[style]?.includes(id)))
 return {strength,opportunities:d,uses:n,qualitySamples:samples,
  independence:instructed&&total>0?1-.3*clamp(individual/total):1}
}
