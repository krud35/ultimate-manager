...
export function makeQueues(smoke = false) {
  ...
  if (smoke) {
    ...
    add('coaches', { homeConfig: candidates[36], points: null, rotate: true })
  ...
  const extras = []
  for (let r = 0; r < 16; r++) for (const instructions of [['throw_hucks', 'no_hucks'], ['play_fast', 'play_slow'], ['poach', 'no_poach'], ['cut_deep', 'cut_under'], ['dump_first', 'safe_throws']])
    for (const variant of [[], instructions.slice(0, 1), instructions.slice(1), instructions]) extras.push({ seed: 9690000 + r,
      control: `conflict-${instructions.join('+')}`, value: variant.join('+') || 'none', homeConfig: { instructions: variant } })
  for (let r = 0; r < 16; r++) for (const trait of ['disciplined', 'huck_lover', 'dump_guy', 'poacher', 'under_cutter', 'deep_threat'])
    for (const enabled of [false, true]) extras.push({ seed: 9695000 + r, control: `trait-${trait}`, value: Number(enabled),
      ...(enabled ? { trait } : {}), homeConfig: { instructions: trait === 'poacher' ? ['no_poach'] : ['no_hucks'] }, points: 2 })
...
export function selectedQueue(phase, configs, rounds) {
  ...
  for (let r = 0; r < rounds; r++) for (const c of configs) for (const swap of [false, true]) {
    ...
    q.push({ id, phase, kind: 'match', candidate: c.id, family: c.family, block: `${c.id}-${r}`, round: r, swap,
      ...
      homeConfig: c, awayConfig: sentinels[(r + Math.floor(r / 6)) % 3], lockWind: true,
