...
function adaptAiSideBeforePoint(session, side, identity = null) {
  ...
  const { tactics, adaptSide } = adaptAiTacticsBetweenPoints({
    ...
    identity: identity ?? resolveAiTeamIdentity(session[side]),
    ...
    rng: session.rng,
    isAi: true,
    // Zmiana za zawodnika zdjętego z linii ma iść po tej samej mierze co auto-sub:
    // najlepszy dostępny i świeży, nie pierwszy z brzegu.
    staminaMap: session.stamina?.[side] ?? null,
  })
...
function preparePointTeams(session, { homeTactics, awayTactics, rotateAway, rotateHome, aiHome, aiAway }) {
  ...
  // Numer punktu idzie do auto-zmian, bo tryb `power_lines` wystawia najlepszą siódemkę
  // co kilka punktów — bez tego nie miałby po czym liczyć.
  const subOpts = { pointIndex: session.pointIndex }
  // Wspólne tylko podczas przygotowania TEGO punktu: uraz, morale i roster mogą
  // zmienić dopasowanie przed kolejnym punktem. Bez cache'u o niejawnej ważności.
  const homeIdentity = aiHome && (rotateHome || session.lastPoint) ? resolveAiTeamIdentity(session.home) : null
  const awayIdentity = aiAway && (rotateAway || session.lastPoint) ? resolveAiTeamIdentity(session.away) : null

  if (rotateHome) {
    session.home.tactics = aiHome
      ? autoRotateTacticsForTeam(session.home, session.stamina.home, session.rng, { ...subOpts, identity: homeIdentity })
      : autoSubstituteTacticsForTeam(session.home, session.stamina.home, subOpts)
  }

  if (rotateAway) {
    session.away.tactics = aiAway
      ? autoRotateTacticsForTeam(session.away, session.stamina.away, session.rng, { ...subOpts, identity: awayIdentity })
      : autoSubstituteTacticsForTeam(session.away, session.stamina.away, subOpts)
  }

  // Adaptacja stylu / instrukcji / reakcja na przeciwnika — po rotacji składu, przed punktem
  if (session.lastPoint) {
    if (aiHome) adaptAiSideBeforePoint(session, 'home', homeIdentity)
    if (aiAway) adaptAiSideBeforePoint(session, 'away', awayIdentity)
  }
...
export function playNextPoint(session, tacticsUpdate = {}, options = {}) {
  ...
  applyFatigueAfterPoint(session, pullBefore, staminaBeforePoint, {
    ...
    durationMs: pointResult.estimatedDurationMs ?? 30000,
  })
  ...
  if (!session.windLocked) session.wind = driftWind(session.wind, session.rng)
