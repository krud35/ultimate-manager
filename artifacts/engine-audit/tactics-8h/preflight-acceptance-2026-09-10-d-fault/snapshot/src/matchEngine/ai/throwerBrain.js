...
import { chooseThrowShape, SHAPE_CALIBRATION } from './throwShape.js'
import {
...
function acceptanceThreshold(stallCount, thrower, tactics = null) {
  ...
  // dumpEarlyBias) i wcześniej dopuszczać reset — bez rozluźniania kryterium innych opcji.
...
function arrivalMarginM(agent, catchPt, defenders, throwerPos, flightSpeedMps = null) {
  ...
  const recvTime = playerTravelSec(agent, catchPt, agent.player ?? agent, 'offense', recvSpeed, 0.65)
  ...
  const recvAtArrival = (recvTime - tFlight) * recvSpeed

  let defAtArrival = Infinity
  for (const d of defenders) {
    const dp = d.player ?? d
    const speed = Math.max(3, sprintSpeedMps(dp))
    const eff = (playerTravelSec(d, catchPt, dp, 'defense', speed, 0.65) - tFlight) * speed
...
function chooseDeliveryPoint(agent, throwerX, throwerY, defenders, possessionTeam, path) {
  ...
  for (let i = 0; i < dists.length; i += 1) {
    ...
    const neededSec = Math.max(0.3, playerTravelSec(agent, pt, agent.player ?? agent, 'offense', recvSpeed, 0.65))
    ...
    if (neededSec * DELIVERY_FEASIBLE_FRAC > tFlight) continue
...
function applyLaneReadToOptions(thrower, considered, { throwerPos, offenseAgents = [], defenseAgents = [], rng, threshold, wind, stallCount, required = false }) {
  if (!throwerPos || !considered.length) return
  ...
  if (!required && !LANE_READ_CALIBRATION.weight) return
  const limit = required ? considered.length : throwLaneReadLimit(thrower)
  ...
  for (const option of considered.slice(0, limit)) {
    if (option.laneRead) continue
    if (!required && option.score < readFloor) break
    ...
    // Rzucający ocenia tor z aktualnie widzianego ustawienia. Wykonanie zachowuje
    // wybrany plan; różnice toru wynikają z błędu celowania i kontroli kształtu.
    const loftStat =
      trajectory === 'deep'
    ...
    if (!shape) { option.score = -Infinity; option.validationStatus = 'missing_plan'; continue }
    if (option.agent) {
      const intercept = selectDiscIntercept({ agent: option.agent, player: option.player, role: 'offense',
        speed: maxSpeedMps(option.player), elapsedMs: 0, totalMs: shape.plan.totalMs + 12000,
        sample: ms => sampleContinuedDisc(shape.plan, ms), blockers: [...offenseAgents, ...defenseAgents] })
      const awareness = subStat(thrower, 'mental', 'decisionMaking') / 100
      const uncertaintyM = ARRIVAL_CALIBRATION.observationRisk ? observationUncertaintyM(option.agent, thrower) : 0
      // With time left, wait for a fresh look instead of aiming from uncertain memory.
      // Under stall pressure the same option remains available, with its risk penalty.
      if (stallCount < 5 && uncertaintyM > ARRIVAL_CALIBRATION.maxUncertaintyM) option.score = -Infinity
      option.score -= uncertaintyM * (6 + 8 * awareness)
      option.score -= Math.max(0, intercept.lateness) * 18 * awareness
      option.score -= Math.max(0, intercept.atMs - shape.plan.totalMs) / 1000 * SHAPE_CALIBRATION.hangCostPerSec
      if (!intercept.legal || (stallCount < 7 && !intercept.reachable)) option.score = -Infinity
      option.validationStatus = !intercept.legal ? 'illegal_contact' : intercept.reachable ? 'reachable' : 'forced_late'
      option.plannedArrival = { reachable: intercept.reachable, legal: intercept.legal,
        atMs: intercept.atMs, lateness: intercept.lateness, support: intercept.support,
        detourSec: intercept.detourSec, avoidedId: intercept.avoidedId, motionGap: intercept.motionGap, uncertaintyM }
    } else {
      option.score = -Infinity
      option.validationStatus = 'missing_receiver'
    }
    option.shape = shape
    option.laneRead = true
    option.score += shape.perceived * LANE_READ_CALIBRATION.weight
  }
}

export const THROW_SCAN_DIAGNOSTICS = { observe: null }

// Detached scalar snapshots: diagnostics never expose live agent objects.
function scanDiagnosticAgents(agents) {
  return agents.map(a => ({ id: a.id ?? a.player?.id, x: a.x, y: a.y,
    vx: a.vx, vy: a.vy, state: a.state, targetX: a.targetX, targetY: a.targetY,
    isDump: a.isDump, observationAgeMs: a.observationAgeMs ?? null,
    visibility: a.visibility ?? null, markTargetId: a.markTargetId ?? null }))
}

export function scanThrowOptions(thrower, offenseAgents, defenseAgents, ctx) {
  const diagnosticActual = THROW_SCAN_DIAGNOSTICS.observe ? {
    offense: scanDiagnosticAgents(offenseAgents), defense: scanDiagnosticAgents(defenseAgents),
  } : null
  const {
    disc,
    stallCount = 1,
  ...
  if (throwerPos) {
    ...
    offenseAgents = perceivePlayers(observer, offenseAgents, blockers, setupElapsedMs,
      { x: throwerPos.x + attackDirectionX(possessionTeam) * 10, y: throwerPos.y })
    defenseAgents = perceivePlayers(observer, defenseAgents, blockers, setupElapsedMs)
  }
  ...
  const rejected = THROW_SCAN_DIAGNOSTICS.observe ? [] : null
  const reject = (agent, reason) => rejected?.push({ id: agent.player.id, reason })
  const options = []
  for (const agent of offenseAgents) {
    ...
    if (agent.state === CUTTER_STATE.CLEARING) {
      ...
      if (Math.hypot(dx, dy) > CLEARING_RESET_MAX_M) { reject(agent, 'clearing'); continue }
    }
    ...
    if (!isDump) {
      if (isEchoBack && forwardProgress < 3 && hardStallCount < 8) { reject(agent, 'echo'); continue }
      // Po serii podań bez postępu jedyną dopuszczalną opcją jest zysk terenu.
      if (requireForwardPass && hardStallCount < 8 && forwardProgress < 2.5) { reject(agent, 'no_progress'); continue }
    }
    ...
    if (
      ...
      reject(agent, 'separation_policy')
      continue
    }
    ...
    score += continuationValue(catchPt, agent.player.id, offenseAgents, defenseAgents,
      attackDirectionX(possessionTeam)) * subStat(thrower, 'mental', 'anticipation') / 100
    score -= Math.min(8, (agent.observationAgeMs ?? 0) / 200)
  ...
  applyLaneReadToOptions(thrower, considered, { throwerPos, offenseAgents, defenseAgents, rng, threshold, wind, stallCount })
  considered.sort((a, b) => b.score - a.score)
  const finish = selected => {
    THROW_SCAN_DIAGNOSTICS.observe?.({ throwerId: thrower.id, setupElapsedMs, hardStallCount, requireForwardPass,
      actual: diagnosticActual, perceived: { offense: scanDiagnosticAgents(offenseAgents), defense: scanDiagnosticAgents(defenseAgents) },
      attackStyle, defenseStyle, possessionTeam, forceSide,
      selectedId: selected?.player?.id ?? null, threshold, rejected,
      players: offenseAgents.map(a => ({ id: a.player.id, state: a.state, stateMs: a.stateMs,
        x: a.x, y: a.y, targetX: a.targetX, targetY: a.targetY, isDump: a.isDump })),
      options: options.map(o => ({ id: o.player?.id, score: o.score, isDump: o.isDump,
        forwardProgress: o.forwardProgress, validationStatus: o.validationStatus,
        reachable: o.plannedArrival?.reachable })) })
    return selected
  }
  ...
  while (considered.length) {
    ...
    if (!best.laneRead) {
      ...
      const maxShapeGain = LANE_READ_CALIBRATION.weight >= 0
        ? (Math.max(0, SHAPE_CALIBRATION.arrivalSideWeight) + Math.abs(SHAPE_CALIBRATION.judgementNoise)) * LANE_READ_CALIBRATION.weight
        : Infinity
      if (best.score + maxShapeGain < threshold - 22) { considered.shift(); continue }
      applyLaneReadToOptions(thrower, [best], { throwerPos, offenseAgents, defenseAgents, rng, threshold,
        wind, stallCount, required: true })
    ...
    const resetTier = stallTier(Math.max(0, stallCount + (throwerMods.resetFirstStallBias ?? 0)))
