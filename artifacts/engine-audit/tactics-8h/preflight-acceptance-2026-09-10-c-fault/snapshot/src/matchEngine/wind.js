...
export function normalizeWind(raw) {
  ...
  const speedMph = clamp(Number(raw.speedMph ?? ((raw.speedMps ?? 0) / MPH_TO_MPS)) || 0, 0, 28)
...
function distanceBucket(distanceM, throwType) {
  if (distanceM != null ? distanceM < 10 : throwType === 'dump_swing') return 'short'
...
export function windThrowModifiers({
  ...
  const fh = getSubStat(thrower?.skills, 'throwing', 'forehand')
  const bh = getSubStat(thrower?.skills, 'throwing', 'backhand')
  const throwing = (fh + bh + getSubStat(thrower?.skills, 'throwing', 'windControl')) / 3
  const composure = getSubStat(thrower?.skills, 'mental', 'composure')
  ...
  // Ciągłe przejście między składową wzdłużną i boczną; etykieta nie zmienia fizyki skokowo.
  const down = Math.max(0, along01) ** 2, up = Math.max(0, -along01) ** 2
  const cross = Math.max(0, 1 - along01 * along01)
  const crossSkillPenalty = (1.25 - skillShield) * (1 + techniqueGap * 0.8)
  const loftExtra = bucket === 'loft' || throwType === 'over_the_top' ? 1.35 : 1
  accuracyDelta = intensity * deepFactor * (down * 3 * skillShield
    - up * 11 * loftExtra * (1.15 - skillShield * 0.4) - cross * 9 * crossSkillPenalty)
  spreadMult = 1 + intensity * (down * 0.05 + up * 0.12 * deepFactor
    + cross * (0.18 + (1 - skillNorm) * 0.2))
  advanceMult = 1 + intensity * (down * (bucket === 'deep' ? 0.14 : bucket === 'medium' ? 0.08 : 0.02)
    - up * (bucket === 'deep' ? 0.18 : bucket === 'medium' ? 0.1 : 0.03))
  dropChanceBonus = intensity * (down * (bucket === 'deep' ? 0.07 : bucket === 'medium' ? 0.035 : 0.01) * (1.15 - skillShield)
    + up * deepFactor * 0.02 + cross * deepFactor * 0.03 * crossSkillPenalty)
