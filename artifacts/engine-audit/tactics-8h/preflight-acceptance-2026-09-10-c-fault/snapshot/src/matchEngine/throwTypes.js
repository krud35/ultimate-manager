...
export function pickThrowType({
  ...
  // Fast-mode equivalents of the spatial style scores; no accuracy bonus.
  weights[THROW_TYPE.DUMP_SWING] *= 1 + (traitMods.swingBias ?? 0) * 0.35
  if (afterTurnover) {
    const bias = traitMods.transitionBias ?? 0
    weights[THROW_TYPE.HUCK] *= 1 + bias * 0.4
    weights[THROW_TYPE.DUMP_SWING] *= 1 - bias * 0.35
  }
...
export function computeThrowAdvance(
  ...
  throwType,
  {
    ...
    explicitYards = null,
    /**
     * Kalibracja fastMode (MATCH_CONFIG.fastCalibration.windMult) — pełny silnik zostawia 1.
     * Bez tego kanał „wiatr zabiera jardy" działał w fastMode z siłą bazową, więc atak
     * pod wiatr nie potrzebował więcej podań na zdobycie punktu — a to właśnie tym
     * mechanizmem pełny silnik zamienia wiatr na przewagę lepszego składu.
     */
  ...
  const windMods =
    windSensitivityMult === 1
      ...
      : {
          ...rawWindMods,
          advanceMult: 1 + ((rawWindMods.advanceMult ?? 1) - 1) * windSensitivityMult,
        }
