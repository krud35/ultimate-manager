...
export function subStat(player, category, key) {
  ...
  return category === 'physical' ? raw : applyMoraleToStat(raw, getPlayerMorale(player))
}

/** Wspólna utrata wydolności ruchowej poniżej 60 energii, z odpornością z cech. */
export function movementFatigueMult(player) {
  const stamina = Math.max(0, Math.min(100, player?.currentStamina ?? 100))
  const resistance = playerMatchMods(player).lowStaminaMovePenaltyMult ?? 1
  return Math.max(0.65, 1 - Math.max(0, 60 - stamina) / 60 * 0.25 * resistance)
}

/** Bazowe Vmax 4.4–7.2 m/s (speed 0–100), następnie cechy i zmęczenie. */
export function maxSpeedMps(player) {
  const mods = playerMatchMods(player)
  return (4.4 + (subStat(player, 'physical', 'speed') / 100) * 2.8) * (mods.speedMult ?? 1) * movementFatigueMult(player)
}
...
export function executeThrowShape(
  ...
  const control = Math.max(0, Math.min(1, (loftStat * 0.4
    + subStat(thrower, 'throwing', 'touch') * 0.35
    + subStat(thrower, 'throwing', 'releaseControl') * 0.25) / 100))
 ...
 *
 * Jawny zasięg ramion lub oszacowanie ze wzrostu. Bez danych używamy 2.2 m;
 * skoczność i morale nie zmieniają długości ciała.
 */

export function standingReachM(player) {
  return playerBody(player).standingReachM
}
...
export function jumpHeightM(player) {
  ...
  return (JUMP_HEIGHT_BASE_M + (jump / 100) * JUMP_HEIGHT_SPAN_M) * movementFatigueMult(player)
}

/**
 * Teoretyczny pułap: zasięg ciała + wybicie. Kontakt używa wysokości aktualnie
 * wykonanego skoku, więc sam ten pułap nie przyznaje zagrania dysku.
 */
...
export function horizontalReachM(player) {
  return Number.isFinite(player?.body?.armSpanCm)
    ? playerBody(player).armSpanM * 0.5 + 0.2 : standingReachM(player) * 0.52
}
