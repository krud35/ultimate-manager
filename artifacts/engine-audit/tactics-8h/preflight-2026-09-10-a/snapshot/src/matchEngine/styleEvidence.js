import { instructionsForPlayer } from './playerInstructions.js'
...
export function instructionContext(tactics, playerId) {
 ...
 return instructionsForPlayer(tactics, playerId).length ? 'individual_instructions' : 'team_system'
}
...
export function captureStyleInstructions(boxScore, players, tactics, role) {
 for (const player of players) {
  ...
  const evidence = row.styleEvidence ??= {version:STYLE_EVIDENCE_VERSION,modes:{}}
  const lineRole = resolveLineRole(tactics)
  const snapshot = {role, lineRole, teamStyle: role==='offense'?tactics?.attackStyle:tactics?.defenseStyle,
   coach: tactics?.[lineRole==='offense'?'oLineCoachDirectives':'dLineCoachDirectives'] ?? null,
   individual: instructionsForPlayer(tactics, player.id)}
