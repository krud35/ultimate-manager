 ...
 */
import { maxSpeedMps, subStat, movementFatigueMult } from './statFormulas.js'
...
export function mobilityMultiplier(player, role = null) {
  ...
  const skill = subStat(player, 'physical', 'agility') * 0.6 + subStat(player, 'physical', 'balance') * 0.4
  const mult = Math.max(
    MOBILITY_MIN,
...
export function integrateAgentMotion(agent, targetX, targetY, maxSpeed, dtSec, limitTurn, role = null, desiredSpeedMps = null) {
  ...
  const requestedSpeed = desiredSpeedMps == null ? maxSpeed : Math.max(0, Math.min(maxSpeed, desiredSpeedMps))
  let targetVx = dirX * requestedSpeed
  let targetVy = dirY * requestedSpeed

  if (dist < maxSpeed * dtSec * 0.5) {
    const approach = dist / Math.max(dtSec, 1e-6)
    targetVx = dirX * Math.min(requestedSpeed, approach)
    targetVy = dirY * Math.min(requestedSpeed, approach)
  }
  ...
  const deltaScale = deltaLength > maxDelta ? maxDelta / deltaLength : 1
  cvx += deltaX * deltaScale
  cvy += deltaY * deltaScale
