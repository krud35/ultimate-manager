...
export function interceptForAgent(flight, agent, player, role, speed) {
  ...
  if (cached && cached.deflection === flight.deflection && elapsedMs - cached.readMs < 200 && cached.target.atMs > elapsedMs) return cached.target
  ...
  const read = Math.max(0, Math.min(1, (elapsedMs - delayMs) / Math.max(400, 1200 - vision * 8)))
...
export function tickFlightContestAgent(agent, intercept, player, role, discSample, rng, speedMult = 1, flight = null) {
  ...
  const approachSpeed = ARRIVAL_CALIBRATION.kinematics && FLIGHT_APPROACH_CALIBRATION.enabled
