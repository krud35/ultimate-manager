...
/** Widok tych samych map w układzie geometrycznym boiska. Nie kopiujemy wartości:
 * zapis ticku ma trafić do właściwej drużyny także po zmianie stron. Sprinty muszą
 * podlegać dokładnie temu samemu przestawieniu co energia.
 */
export function staminaMapsForGeometry(stamina, sidesSwapped) {
  if (!stamina || !sidesSwapped) return stamina
  const sprintM = ensureSprintMeters(stamina)
  return { ...stamina, home: stamina.away, away: stamina.home,
    sprintM: { home: sprintM.away, away: sprintM.home } }
}
...
export function buildPointPlayersById(offenseLineup, defenseLineup) {
  const byId = {}
...
/** One execution penalty, shared by detailed and background throws. Movement,
 * perception and catching retain their own fatigue effects. Design calibration;
 * not an empirical physiological fit. */
export const THROW_FATIGUE_CALIBRATION = { singleExecution: true, maximumPenalty: 24, exponent: 1.5 }
export function throwFatiguePenalty(stamina) {
  const s = Math.max(0, Math.min(100, stamina ?? 100))
  if (!THROW_FATIGUE_CALIBRATION.singleExecution) return staminaPerformancePenalty(s)
    + staminaThrowCollapsePenalty(s) + motionFatigueModifiers(s).throwAccuracyPenalty
  return THROW_FATIGUE_CALIBRATION.maximumPenalty * Math.pow(Math.max(0, (80 - s) / 80), THROW_FATIGUE_CALIBRATION.exponent)
}
