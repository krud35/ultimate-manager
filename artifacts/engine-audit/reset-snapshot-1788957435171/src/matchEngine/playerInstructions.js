...
/**
 * Które założenia trenerskie rozkaz PRZEJMUJE dla tego zawodnika.
 *
 * Rozkaz indywidualny nie sumuje się z dyrektywą na tej samej osi — wypiera ją
 * (patrz effectiveCoachDirectives). Dzięki temu „cała linia nie poachuje, ale ten jeden
 * ma zielone światło" znaczy dokładnie to, co mówi, zamiast dawać wypadkową obu ustawień.
 *
 * Pary bez odpowiednika wśród dyrektyw (cut_deep, dominate, take_space…) nie przejmują nic —
 * poruszają osie, których trener nie ma na suwakach.
 */
const INSTRUCTION_OVERRIDES = {
  throw_hucks: ['huckAppetite'],
  no_hucks: ['huckAppetite'],
  break_mark: ['breakAppetite'],
  no_break_mark: ['breakAppetite'],
  safe_throws: ['passSelectivity', 'creativity'],
  take_risks: ['passSelectivity', 'creativity'],
  play_fast: ['possessionTempo'],
  play_slow: ['possessionTempo'],
  tight_mark: ['cushionDepth'],
  loose_mark: ['cushionDepth'],
  shade_deep: ['coverageShade'],
  shade_under: ['coverageShade'],
  poach: ['poachSeeking'],
  no_poach: ['poachSeeking'],
}
