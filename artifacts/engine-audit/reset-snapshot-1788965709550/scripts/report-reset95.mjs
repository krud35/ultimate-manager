...
const names = ['control', 'safe', 'layout', 'combined', 'execution', 'moderate', 'errors14', 'errors10', 'validation', 'defaults', 'fast-validation']
const rows = []
...
assert.ok(summary.completionPct >= 90 && summary.completionPct <= 93, 'Overall completion outside the previous target')
...
const scenarios = JSON.parse(fs.readFileSync('artifacts/scenarios/reset95-errors14.json')).summaries.filter(r => r.layer === 'execution')
fs.writeFileSync(`${dir}/RESET-94-96.md`, [
  ...
  'Sprawdzenie geometrii 4 dodatkowych meczów: 1090 chwytów i 85 strat; poprawne kontakty, ciągłość ruchu i brak sztucznych zakończeń punktów. Sprawdzenia dobiegu, percepcji, omijania, resetów, 33 atrybutów i realizmu oraz ESLint: OK. Produkcyjny build: OK, z ostrzeżeniem o dużych chunkach.', '',
