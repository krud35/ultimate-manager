...
const window = { agent, player, role: 'offense', speed: 7, elapsedMs: 0, totalMs: 180,
  sample: ms => ({ x: support.x + (180 - ms) * 0.02, y: support.y, z: 1.1 }) }
assert.equal(selectDiscIntercept(window).reachable, false)
assert.equal(selectDiscIntercept({ ...window, agent: { ...agent, vx: 0, vy: 0 } }).reachable, true)
ARRIVAL_CALIBRATION.kinematics = false
try { assert.equal(selectDiscIntercept(window).reachable, true, 'Legacy planner falsely accepts this window') }
finally { ARRIVAL_CALIBRATION.kinematics = true }
