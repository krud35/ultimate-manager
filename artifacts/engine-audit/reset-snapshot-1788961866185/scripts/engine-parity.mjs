 ...
 * roboczymi hipotezami kalibracji club ultimate. Pasma nie są zweryfikowanymi normami
 * EUCF/USAU/WUCC; nie należy przedstawiać ich jako danych z tych zawodów.
 *
 * Użycie:
...
/**
 * Hipotezy kalibracyjne dla club ultimate, NIE zweryfikowane normy zawodów.
 * Dystanse dotyczą udanych podań. Hold nie jest skutecznością pierwszego posiadania.
 * Ocena realizmu wymaga także warunków, rodzajów strat i porównania obu ścieżek.
 */
/**
...
function measure(matches, fastMode) {
  ...
  for (let i = 0; i < matches; i += 1) {
    ...
    const t0 = Date.now()
    initialWindMph += generateWind(createRng(seedFor(i))).speedMph
  ...
  return {
    initialWindMph: matches ? initialWindMph / matches : null,
...
for (const [label, key] of rows) {
  ...
  if (band && Number.isFinite(f)) measuredTargets++
  if (band && Number.isFinite(u)) measuredTargets++
  if (gap != null && band) {
...
console.log(`\nkomórek poza orientacyjnym pasmem: ${offTarget} / ${measuredTargets} zmierzonych`)
console.log(`początkowy wiatr: fast ${fmt(fast.initialWindMph)} mph, pełny ${fmt(full.initialWindMph)} mph; dystanse obejmują udane podania`)
