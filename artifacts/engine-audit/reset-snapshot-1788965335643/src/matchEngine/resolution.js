...
export function resolveThrow({
  ...
  executionOnly = false,
  marker = null,
  markerDistanceM = Infinity,
  /**
   * Kalibracja fastMode (MATCH_CONFIG.fastCalibration) — pełny silnik zostawia domyślne
   * 1 / 0 / 1 i zachowuje się dokładnie jak dotąd. Patrz komentarz przy fastCalibration.
   */
  skillSensitivityMult = 1,
  gapOffset = 0,
  windSensitivityMult = 1,
  windSkillSlope = 0,
  windSkillRef = 83,
  /** Tylko do kalibracji (tmp-calibrate-*.mjs) — pomija tabelę DISTANCE_GAP_TABLE. */
  __debugGapOverride = null,
}) {
  if (executionOnly) defender = markerDistanceM < 3 ? marker : null
  const { skillCheck } = MATCH_CONFIG
  const profile = throwProfile(throwType)
  const rawWindMods = windThrowModifiers({
    wind,
    throwDx,
    throwDy,
    throwType,
  ...
  const windMods =
    windSensitivityMult === 1
      ...
      : {
          ...rawWindMods,
          accuracyDelta: (rawWindMods.accuracyDelta ?? 0) * windSensitivityMult,
          dropChanceBonus: (rawWindMods.dropChanceBonus ?? 0) * windSensitivityMult,
          spreadMult: 1 + ((rawWindMods.spreadMult ?? 1) - 1) * windSensitivityMult,
        }
  ...
  const stallMods = stallThrowModifiers({
    ...
    defender,
    stallCount,
    separation: executionOnly ? { outcome: markerDistanceM < 1.3 ? 'tight' : markerDistanceM < 3 ? 'contested' : 'open' } : separation,
    rng,
  ...
  //    fastMode wchodzi przez resolveSeparation (timing, przyspieszenie, szybkość i przewidywanie).
  //
  ...
  const throwStat =
    throwSkill * 0.75 + subStat(thrower, 'throwing', 'touch') * 0.15
      + subStat(thrower, 'throwing', 'releaseControl') * 0.1

  /**
   * Przechył kary za wiatr wg umiejętności rzucającego (kalibracja fastMode; pełny
   * silnik zostawia windSkillSlope = 0 i nic tu nie zmienia). Wokół `windSkillRef`
   * średnia kary zostaje ta sama — zmienia się jej ROZKŁAD: słabszy rzucający dostaje
   * więcej, lepszy mniej. Patrz MATCH_CONFIG.fastCalibration.windSkillSlope.
   */
  const windTilt = windSkillSlope
    ? Math.max(0.15, 1 - windSkillSlope * ((throwStat - windSkillRef) / 50))
    : 1
  const windControl = 1.4 - subStat(thrower, 'throwing', 'windControl') / 100 * 0.6
  const windAccuracyDelta = (windMods.accuracyDelta ?? 0) * windTilt * windControl
  const windDropChanceBonus = (windMods.dropChanceBonus ?? 0) * windTilt * windControl
  ...
  const baseGap =
    (__debugGapOverride ?? distanceGapFor(distanceCategory, isOpenSide, executionOnly ? 'open' : separation?.outcome)) +
    gapOffset - Math.max(0, distanceM - (20 + subStat(thrower, 'throwing', 'power') * 0.4)) * 0.45

  const throwBase =
    baseGap +
    skillAdjustment(throwStat, distanceCategory, isOpenSide) * skillSensitivityMult -
  ...
  const defenseBase =
    (compressSkill((subStat(defender, 'defensive', 'blocking') * 0.6 + subStat(defender, 'defensive', 'marking') * 0.4)) - 50) * 0.12 +
  ...
  let throwScore =
    ...
    throwFatiguePenalty(thrower.currentStamina ?? 100)
  const styleDefMult =
  ...
  if (executionOnly) {
    const proximity = Math.max(0, 1 - markerDistanceM / 3)
    defenseScore = defender ? proximity * (subStat(defender, 'defensive', 'marking') * 0.05
      + (defenderTraits.markPressureBonus ?? 0) * 0.3) : 0
    return { success: throwScore > defenseScore, throwScore: Math.round(throwScore),
      defenseScore: Math.round(defenseScore), throwStat: Math.round(throwStat), isBlock: false,
      isDrop: false, isLaneBlock: false, isWindDrop: false, throwTechnique: technique,
      windRelation: windMods.relation, windAdvanceMult: windMods.advanceMult,
      execution: { markerPressure: defenseScore, windPenalty: windAccuracyDelta,
        fatiguePenalty: throwFatiguePenalty(thrower.currentStamina ?? 100),
        fatigueComponents: { execution: throwFatiguePenalty(thrower.currentStamina ?? 100) } } }
  }
  ...
  if (success && windDropChanceBonus > 0 && rng?.float) {
    if (rng.float() < windDropChanceBonus) {
      success = false
      isBlock = false
      isWindDrop = true
    }
...
export const MISS_CALIBRATION = {
  // Margin controls situational displacement; skill also controls rare aim errors,
  // arc/curve execution and wind compensation. It does not grant a completed pass.
  // Recalibrated after removing unnecessary offensive layouts: RESET-94-96.md.
  // A misplaced disc remains recoverable; this does not directly roll a turnover.
  cleanMargin: 12,
   ...
   *  do minMissChance. throwStat 60 -> 14%, 80 -> 7.78%, 95 -> 3.11%. */
  missChanceAtPivot: 0.14,
  minMissChance: 0.015,
  skillMissPivot: 60,
  skillMissSpan: 45,
  /** Pomyłka przesuwa punkt dostarczenia o 1.8–4 m, dalej ograniczane długością rzutu. */
  errorMissMinM: 1.8,
  errorMissSpanM: 2.2,
}
